...
def create_route(route: RouteCreate, user_id: int, db: Session = Depends(get_db)):
    new_route = Route(
        user_id=user_id,
        title=route.title,
        theme=route.theme,
...
def get_user_routes(user_id: int, db: Session = Depends(get_db)):
    routes = db.query(Route).filter(Route.user_id == user_id).all()
    if not routes:
        raise HTTPException(status_code=404, detail="여정이 없습니다")
    return routes
