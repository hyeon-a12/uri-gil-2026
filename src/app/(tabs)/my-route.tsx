...
function TravelShareSheet({
  ...
  const handleSaveScheduleImage = async () => {
    ...
    try {
      ...
      // 웹은 갤러리가 없어서(expo-media-library 웹 미지원), 캡처된 이미지를
      // 브라우저 기본 다운로드로 대체합니다 — 별도 권한 절차 없이 바로
      // 기기의 다운로드 폴더에 저장됩니다. react-native-view-shot의 웹
      // 구현은 uri로 data: URL을 돌려주므로 그대로 <a download>에 씁니다.
      if (Platform.OS === 'web') {
        const link = document.createElement('a');
        link.href = uri;
        link.download = `${tripName || 'urigil'}-일정표.png`;
