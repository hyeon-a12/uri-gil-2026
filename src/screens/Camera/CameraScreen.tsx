...
import { getAllFolders, type ShootingStyleId } from '@/services/folderService';
        ...
        try {
          ...
          try {
            ...
            if (folder?.routeId) {
              ...
              let clipUrl = video.uri;
              try {
                clipUrl = await uploadClipVideo(video.uri);
              } catch (uploadError) {
                console.warn('[Camera] 영상 업로드 실패, 로컬 경로로 폴백:', uploadError);
              }
              ...
              if (record.thumbnail) {
                try {
                  ...
                  console.warn('[Camera] 썸네일 업로드 실패:', thumbnailError);
                }
              }
            ...
            console.error('[Camera] 서버 클립 저장 실패:', serverError);
          }
